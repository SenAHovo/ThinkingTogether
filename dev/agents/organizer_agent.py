...
class OrganizerAgent:
    ...
    def _build_opening_chain():
        sys = (
            ...
            "- 学习概念：从问题出发，激发探索欲望\n"
            "- 课程学习：梳理知识结构，安排学习路径，识别重点难点\n"
            "- 编程问题：分解技术任务，组织讨论重点，平衡理论与实践\n"
            "- 科学探索：设计研究流程，引导假设验证，分析实验结果\n"
            "\n"
    ...
    def route(self, state: Any, transcript_tail: List[dict]) -> Dict[str, Any]:
        ...
        instruction = (
            ...
            "\n"
            "【课程学习话题】\n"
            "1. 梳理知识体系→理论家：分析概念关系、构建学习路径\n"
            "2. 提供学习方法→实践者：给出具体技巧、练习项目\n"
            "3. 识别学习陷阱→质疑者：指出常见误解、评估掌握标准\n"
            "\n"
            "【编程话题】\n"
            "1. 分析架构原理→理论家：讲解设计模式、算法原理\n"
            "2. 给出实现方案→实践者：写代码、选库、调试\n"
            "3. 审查代码逻辑→质疑者：找bug、质疑设计、review\n"
            "\n"
            "【科学探索话题】\n"
            "1. 提出理论假设→理论家：构建框架、解释机制\n"
            "2. 设计实验方案→实践者：实验步骤、测量方法\n"
            "3. 检验假设逻辑→质疑者：挑战前提、质疑实验设计\n"
            "\n"
            "你只输出 JSON，格式必须严格如下：\n"
            "{"
            ...
            f"上一发言者：{avoid or '（无）'}\n"
            "请仔细分析最近2-3轮对话，判断话题类型，找出需要回应的具体观点。\n"
